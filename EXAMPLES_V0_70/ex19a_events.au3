...
Func MainFunc()
  ...
  If $oUIAutomation.AddAutomationEventHandler( $UIA_Invoke_InvokedEventId, $pUIElement, $TreeScope_Subtree, 0, $oIUIAutomationEventHandler ) Then Exit
  If $oUIAutomation.AddAutomationEventHandler( $UIA_LayoutInvalidatedEventId, $pUIElement, $TreeScope_Subtree, 0, $oIUIAutomationEventHandler ) Then Exit
  If $oUIAutomation.AddAutomationEventHandler( $UIA_Text_TextSelectionChangedEventId, $pUIElement, $TreeScope_Subtree, 0, $oIUIAutomationEventHandler ) Then Exit
If $oUIAutomation.AddAutomationEventHandler( $UIA_Text_TextChangedEventId, $pUIElement, $TreeScope_Subtree, 0, $oIUIAutomationEventHandler ) Then Exit
If $oUIAutomation.AddAutomationEventHandler( $UIA_InputReachedTargetEventId, $pUIElement, $TreeScope_Subtree, 0, $oIUIAutomationEventHandler ) Then Exit
If $oUIAutomation.AddAutomationEventHandler( $UIA_InputReachedOtherElementEventId, $pUIElement, $TreeScope_Subtree, 0, $oIUIAutomationEventHandler ) Then Exit
If $oUIAutomation.AddAutomationEventHandler( $UIA_InputDiscardedEventId, $pUIElement, $TreeScope_Subtree, 0, $oIUIAutomationEventHandler ) Then Exit
...
;~   ConsoleWrite( "oIUIAutomationEventHandler_HandleAutomationEvent: " & $iEventId & @CRLF )
	...
	local $handle =Hex( _UIA_getPropertyValue( $oSender, $UIA_NativeWindowHandlePropertyId ) )
;~ only interest in gui visible window events
	If $handle<>0 then
;~ 		$iEventId <> $UIA_Window_WindowClosedEventId Then


    ConsoleWrite( "$iEventId    = " & $iEventId & @tab & _
				  "Title     = " & _UIA_getPropertyValue( $oSender, $UIA_NamePropertyId ) & @tab & _
